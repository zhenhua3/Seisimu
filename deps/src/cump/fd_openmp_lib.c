#include<stdio.h>
 ...
 //********************************************//
double fd(double *in_wf, int idx0_in_wf, int idx1_in_wf, int idx2_in_wf, int idx3_in_wf, double *fdc)
 {
  double tmp;
  ...
  double body_x(double *out_wf, int BD_nz_out, int BD_nx_out, int BD_ny_out,
                  ...
                  double dt, double *fdc)
 {
  ...
  tmp = fd(in_wf, idx0_in_wf, idx1_in_wf, idx2_in_wf, idx3_in_wf, fdc);
  tmp = tmp / dx * para * dt;
  *(out_wf+idx_out_wf) = *(out_wf+idx_out_wf) + tmp;
  return tmp;
   ...
   double body_y(double *out_wf, int BD_nz_out, int BD_nx_out, int BD_ny_out,
                   int idx_i_out, int idx_j_out, int idx_k_out,
                   double *in_wf, int BD_nz_in, int BD_nx_in,
  ...
  double body_z(double *out_wf, int BD_nz_out, int BD_nx_out, int BD_ny_out,
                  int idx_i_out, int idx_j_out, int idx_k_out,
                  double *in_wf, int BD_nz_in, int BD_nx_in,
  ...
  *(out_wf+idx_out_wf) = *(out_wf+idx_out_wf) + tmp ;
  ...
  void bound_x(double *out_wf, int BD_nz_out, int BD_nx_out, int BD_ny_out,
                  int idx_i_out, int idx_j_out, int idx_k_out,
                  double *in_wf, int BD_nz_in, int BD_nx_in,
                  int idx_j_in_start,
                  ...
                  double *fdc)
  {
   ...
   tmp_head = body_x(out_wf, BD_nz_out, BD_nx_out, BD_ny_out,
   ...
   *(out_wf+idx_out_wf_head) = *(out_wf+idx_out_wf_head) + *(BDwf+idx_BDwf_head);
   ...
   tmp_toe = body_x(out_wf, BD_nz_out, BD_nx_out,BD_ny_out,
   ...
   *(out_wf+idx_out_wf_toe) = *(out_wf+idx_out_wf_toe) + *(BDwf+idx_BDwf_toe);
   ...
   void bound_y(double *out_wf, int BD_nz_out, int BD_nx_out, int BD_ny_out,
                   int idx_i_out, int idx_j_out, int idx_k_out,
                   double *in_wf, int BD_nz_in, int BD_nx_in,
                   int idx_k_in_start,
                   ...
                   double *fdc)
   {
    ...
    double tmp_head, tmp_toe;
    int idx_out_wf_head, idx_out_wf_toe, idx_BDwf_head, idx_BDwf_toe;
    tmp_head = body_y(out_wf, BD_nz_out, BD_nx_out, BD_ny_out,
           idx_i_out, idx_j_out, idx_k_out,
    ...
    *(out_wf+idx_out_wf_head) = *(out_wf+idx_out_wf_head) + *(BDwf+idx_BDwf_head);
    ...
    tmp_toe = body_y(out_wf, BD_nz_out, BD_nx_out, BD_ny_out,
    ...
    *(out_wf+idx_out_wf_toe) = *(out_wf+idx_out_wf_toe) + *(BDwf+idx_BDwf_toe);
    ...
    void unlimited_bound_z(double *out_wf, int BD_nz_out, int BD_nx_out, int BD_ny_out,
                    int idx_i_out, int idx_j_out, int idx_k_out,
                    double *in_wf, int BD_nz_in, int BD_nx_in,
                    int idx_i_in_start,
                    ...
                    double *fdc)
    {
     ...
     double tmp_head, tmp_toe;
     int idx_out_wf_head, idx_out_wf_toe, idx_BDwf_head, idx_BDwf_toe;
     tmp_head = body_z(out_wf, BD_nz_out, BD_nx_out, BD_ny_out,
            idx_i_out, idx_j_out, idx_k_out,
     ...
     *(out_wf+idx_out_wf_head) = *(out_wf+idx_out_wf_head) + *(BDwf+idx_BDwf_head);
     ...
     tmp_toe = body_z(out_wf, BD_nz_out, BD_nx_out, BD_ny_out,
     ...
     *(out_wf+idx_out_wf_toe) = *(out_wf+idx_out_wf_toe) + *(BDwf+idx_BDwf_toe);
     ...
     void free_bound_z(double *out_wf, int BD_nz_out, int BD_nx_out, int BD_ny_out,
                    int idx_i_out, int idx_j_out, int idx_k_out,
                    double *in_wf, int BD_nz_in, int BD_nx_in,
                    ...
                    double *fdc)
    {
     ...
     tmp_toe = body_z(out_wf, BD_nz_out, BD_nx_out, BD_ny_out,
            idx_i_out, BD_nz_out - idx_k_out - 1, idx_k_out,
            in_wf, BD_nz_in, BD_nx_in, idx_i_in_start, para_toe, dz, dt, fdc);
 ...
 //********************************************//
 void body_tpp_x(double *out_txx, double *out_tyy, double *out_tzz,
    ...
    {
     double tmp, tmp_txx, tmp_tyy, tmp_tzz;
     int idx_out_tpp, idx0_in_wf, idx1_in_wf, idx2_in_wf, idx3_in_wf;
     idx_out_tpp = idx_i_out + idx_j_out * BD_nz_out + idx_k_out * BD_nz_out * BD_nx_out;
     idx0_in_wf = idx_i_out + (idx_j_out - idx_j_in_start) * BD_nz_in + idx_k_out * BD_nz_in * BD_nx_in;
     idx1_in_wf = idx_i_out + (idx_j_out - idx_j_in_start + 1) * BD_nz_in + idx_k_out * BD_nz_in * BD_nx_in;
     idx2_in_wf = idx_i_out + (idx_j_out - idx_j_in_start + 2) * BD_nz_in + idx_k_out * BD_nz_in * BD_nx_in;
     idx3_in_wf = idx_i_out + (idx_j_out - idx_j_in_start + 3) * BD_nz_in + idx_k_out * BD_nz_in * BD_nx_in;
     ...
     tmp_tyy = tmp / dx * *(lambda+idx_out_tpp) * dt;
     tmp_tzz = tmp_tyy;

     *(out_txx+idx_out_tpp) = *(out_txx+idx_out_tpp) + tmp_txx;
     *(out_tyy+idx_out_tpp) = *(out_tyy+idx_out_tpp) + tmp_tyy;
     *(out_tzz+idx_out_tpp) = *(out_tzz+idx_out_tpp) + tmp_tzz;
    }

    void body_tpp_y(double *out_txx, double *out_tyy, double *out_tzz,
                     int BD_nz_out, int BD_nx_out, int BD_ny_out,
                     int idx_i_out, int idx_j_out, int idx_k_out,
                     double *in_wf, int BD_nz_in, int BD_nx_in,
                     int idx_k_in_start, double *lambda, double *mu,
                     double dy, double dt, double *fdc)
       {
        double tmp, tmp_txx, tmp_tyy, tmp_tzz;
        int idx_out_tpp, idx0_in_wf, idx1_in_wf, idx2_in_wf, idx3_in_wf;
        idx_out_tpp = idx_i_out + idx_j_out * BD_nz_out + idx_k_out * BD_nz_out * BD_nx_out;
        ...
        tmp = fd(in_wf, idx0_in_wf, idx1_in_wf, idx2_in_wf, idx3_in_wf, fdc);
        tmp_txx = tmp / dy * *(lambda+idx_out_tpp) * dt;
        tmp_tyy = tmp / dy * (*(lambda+idx_out_tpp) + 2**(mu+idx_out_tpp)) * dt;
        tmp_tzz = tmp_txx;
       ...
       void body_tpp_z(double *out_txx, double *out_tyy, double *out_tzz,
                        int BD_nz_out, int BD_nx_out, int BD_ny_out,
                        int idx_i_out, int idx_j_out, int idx_k_out,
                        double *in_wf, int BD_nz_in, int BD_nx_in,
        ...
        void bound_tpp_x(double *out_txx, double *out_tyy, double *out_tzz,
                         int BD_nz_out, int BD_nx_out, int BD_ny_out,
                         int idx_i_out, int idx_j_out, int idx_k_out,
                         double *in_wf, int BD_nz_in, int BD_nx_in,
          ...
          *(out_tyy+idx_out_head_tpp) = *(out_tyy+idx_out_head_tpp) + tmp_head_tyy + *(BDwf_tyy+idx_BDwf_head);
          ...
          *(out_tyy+idx_out_toe_tpp) = *(out_tyy+idx_out_toe_tpp) + tmp_toe_tyy + *(BDwf_tyy+idx_BDwf_toe);
          *(out_tzz+idx_out_toe_tpp) = *(out_tzz+idx_out_toe_tpp) + tmp_toe_tzz + *(BDwf_tzz+idx_BDwf_toe);
         }

         void bound_tpp_y(double *out_txx, double *out_tyy, double *out_tzz,
                          int BD_nz_out, int BD_nx_out, int BD_ny_out,
                          int idx_i_out, int idx_j_out, int idx_k_out,
                          double *in_wf, int BD_nz_in, int BD_nx_in,
          ...
          void unlimited_bound_tpp_z(double *out_txx, double *out_tyy, double *out_tzz,
                           int BD_nz_out, int BD_nx_out, int BD_ny_out,
                           int idx_i_out, int idx_j_out, int idx_k_out,
                           double *in_wf, int BD_nz_in, int BD_nx_in,
           ...
           void free_bound_tpp_z(double *out_txx, double *out_tyy, double *out_tzz,
                            int BD_nz_out, int BD_nx_out, int BD_ny_out,
                            int idx_i_out, int idx_j_out, int idx_k_out,
                            double *in_wf, int BD_nz_in, int BD_nx_in,
                            int idx_i_in_start, double *lambda, double *mu, double dz,
